...
class Layer(lemon.BaseAgentLemon):
    ...
    def _start(self, k, path, args):
        c    = Contractor(self._logger, self._config, self._contractors[k], path, args)
        c.start()
...
class Contractor(lemon.BaseAgentLemon):
    ...
    def __init__(self, _logger, _config, _place, path, args):
        self._place = _place
        self._call_args = list(path).append(args)
    ...
    def run(self):
        ...
        self.out
        self.err
        exit_code   = self.subprocces = subprocess.call(self._call_args, stdout = self.out, stderr = self.err)
